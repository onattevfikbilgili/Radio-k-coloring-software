...
Coloring mixed_ilp(unsigned int k, Graph G){
	...
	IloNumVarArray x(myenv, n * (upper_bound + 1), 0, 1, ILOINT);//variables x_{v,i} representing whether the vertex v has color i
	...
	//Program the variables to mach data from y and z
	for(int v = 0; v < n; v++){
		for(int i = 0; i <= upper_bound; i++){
			mymodel.add(IloRange(myenv,1,x[v * (upper_bound + 1) + i] + y[v * (upper_bound + 1) + i]+ z[v * (upper_bound + 1) + i],1));
		}
	}
			...
			if(v > w){
				for(int color1 = 0; color1 <= upper_bound; color1++){
					for(int color2 = bigger(0,color1 - (k - dist)); color2 <= smaller(upper_bound, color1 + (k - dist)); color2++){//finds incompatible pairs of colors on the vertices at distance less than k
						mymodel.add(IloRange(myenv,0,x[v * (upper_bound + 1) + color1] + x[w * (upper_bound + 1) + color2],1));
					}
				}
			}
		}
